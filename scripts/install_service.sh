...
for var in FINBLADE_API_KEY FINBLADE_INTEGRATION_KEY FINBLADE_SITE_ID \
           ...
           FINBLADE_MCP_TOKEN FINBLADE_MCP_PORT FINBLADE_MCP_SEARCH_KEY \
           FINBLADE_TOPOLOGY FINBLADE_PUBLIC_URL \
           FINBLADE_PRESENCE_UNMATCHED_EXIT FINBLADE_PRESENCE_EXPIRE_HOURS; do
  value="${!var-}"
  if [ -n "$value" ]; then
    set_env "$var" "$value"
    case "$var" in
